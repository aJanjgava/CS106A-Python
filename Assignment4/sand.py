...
def check_move(grid, x1, y1, x2, y2):
    ...
    if not (0 <= x2 < len(grid[0]) and 0 <= y2 < len(grid)):
        return False
    if not grid[y2][x2] == None:
        return False
    if (((y2 == y1 + 1 and x2 == x1 + 1) and (grid[y1][x1 + 1] != None)) or ((y2 == y1 + 1 and x2 == x1 - 1)) and (
            grid[y1][x1 - 1] != None)):
        return False
    else:
        return True
...
def do_gravity(grid, x, y):
    ...
    if grid[y][x] != 's':
        return grid
    if grid[y][x] == 's':  # check down move
        x2 = x
        y2 = y + 1
        if check_move(grid, x1, y1, x2, y2) == True:
            return do_move(grid, x1, y1, x2, y2)

        elif grid[y][x] == 's':  # Check down-left
            x2 = x - 1
            y2 = y + 1
            if check_move(grid, x1, y1, x2, y2) == True:
                return do_move(grid, x1, y1, x2, y2)

            elif grid[y][x] == 's':  # check down-right
                x2 = x + 1
                y2 = y + 1
                if check_move(grid, x1, y1, x2, y2) == True:
                    return do_move(grid, x1, y1, x2, y2)
...
def big_erase(grid, x, y, canvas):
    ...
    for ey in range(y1, y2 + 1):
        for ex in range(x1, x2 + 1):
            # circle around x,y
            if not (ex < 0 or ex >= len(grid[0]) or ey < 0 or ey >= len(grid)) and abs(x - ex) ** 2 + abs(
                    y - ey) ** 2 <= rad ** 2:
                grid[ey][ex] = None
