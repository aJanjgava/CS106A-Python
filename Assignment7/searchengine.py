...
def create_index(filenames, index, file_titles):
    ...
    {'test1.txt': 'File 1 Title'}
    """
    # For TITLE documentation
    for i in range(len(filenames)):
        filename = open(filenames[i])
        title = next(filename).strip(string.punctuation).strip()
        file_titles[filenames[i]] = title

    # For INDEX documentation
    for i in range(len(filenames)):
        filename = open(filenames[i])
        for line in filename:
            ...
            for m in range(len(line)):
                line[m] = line[m].lower()
                line[m] = line[m].strip(string.punctuation)
                if (line[m] not in index) and (len(line[m]) > 0):
                    index[line[m]] = [filenames[i]]
                elif (line[m] in index) and (filenames[i] not in index[line[m]]):
                    index[line[m]].append(filenames[i])
...
def search(index, query):
    ...
    for i in range(len(query_list)):
        word = query_list[i]
        for term in index:
            if word == term and (len(query_list) < 2):
                return index[term]
            elif word == term and (len(query_list) >= 2):
                posting_list.append(index[term])

    # Return useful List
    compare_list = posting_list[0]
    for i in range(1, len(posting_list)):
        new_list = common(compare_list, posting_list[i])
        compare_list = new_list
    for elem in index:
        if compare_list == index[elem]:
            return index[elem]
